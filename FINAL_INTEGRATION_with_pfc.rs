...
fn main() {
    println!("--- Neurochemical Personality Simulation: Integrated System ---");
    
    let agent0 = PersonalityAgent::new(0, true, false, true); 
    let agent1 = PersonalityAgent::new(1, false, true, false); 
    let agent2 = PersonalityAgent::new(2, false, false, false); 

    let agents = vec![agent0, agent1, agent2];
    let world = GridWorld::new((10, 10));
    let mut multi_world = MultiAgentWorld::new(agents, world);

    println!("\nAgents Initialized:");
    println!("Agent 0 (HRL+ToM) | Agent 1 (Meta-L) | Agent 2 (A2C)");
    ...
    for step in 0..NUM_STEPS {
        ...
        for i in 0..multi_world.agents.len() {
            ...
            println!(
                "Agent {}: Type={}, Action={}, Dopamine={:.2}, Serotonin={:.2}, Skill={}",
                i, decision_type, agent.last_action, agent.neurochemistry.dopamine, agent.neurochemistry.serotonin, skill_id
            );
        }
    }
